...
function slugToName(slug: string): string {
  // URL slug → 검색용 패턴 (bts → BTS, blackpink → BLACKPINK 등은 DB ilike로 처리)
...
async function getArtistData(slug: string): Promise<ArtistData | null> {
  ...
  const { data: tracks } = await supabase
    .from('tracks').select('id, title').in('artist_id', artistIds)
  if (!tracks || tracks.length === 0) return { ...artist, songs: [] }
  ...
  const stats: Record<number, SongStat> = {}
  for (const e of allEntries) {
    ...
    if (!stats[tid]) {
      ...
      stats[tid].score += 101 - e.rank
      if (e.chart_date > stats[tid].last_chart_date) stats[tid].last_chart_date = e.chart_date
    }
  }

  const songs = Object.values(stats).sort((a, b) => b.score - a.score || a.peak_rank - b.peak_rank)
  return { ...artist, songs }
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const bestPeak = data.songs.length > 0 ? data.songs[0].peak_rank : null
  const title = `${data.name} — K-pop on Billboard Hot 100`
  const description = bestPeak
    ? `${data.name}'s Billboard Hot 100 chart history. Peak #${bestPeak} · ${data.songs.length} songs charted · ${data.songs.reduce((a, s) => a + s.total_weeks, 0)} total weeks.`
    : `${data.name}'s Billboard Hot 100 chart history.`

  return {
    title,
    description,
    alternates: { canonical: `${BASE_URL}/${locale}/kpop/${slug}` },
    openGraph: { title, description, url: `${BASE_URL}/${locale}/kpop/${slug}` },
  }
}
...
export default async function ArtistPage({ params }: Props) {
  ...
  const totalWeeks = data.songs.reduce((a, s) => a + s.total_weeks, 0)
  const totalScore = data.songs.reduce((a, s) => a + s.score, 0)
  const bestPeak = data.songs.length > 0 ? data.songs[0].peak_rank : null
  ...
  const jsonLd = {
    ...
    '@type': 'ItemList',
    name: `${data.name} — Billboard Hot 100`,
    description: `${data.name}'s complete Billboard Hot 100 chart history`,
    url: `${BASE_URL}/${locale}/kpop/${slug}`,
    numberOfItems: data.songs.length,
    itemListElement: data.songs.map((s, i) => ({
      '@type': 'ListItem',
      position: i + 1,
      name: s.title,
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
      ...
      <Link href={`/kpop`} className="text-zinc-500 text-sm hover:text-white transition-colors mb-6 block">
        ← {t('kpop.title')}
      </Link>

      <div className="mb-8">
        <p className="text-xs text-pink-400 font-medium mb-1">🇰🇷 K-pop on Billboard Hot 100</p>
        <h1 className="text-4xl font-bold text-white mb-3">{data.name}</h1>
        {bestPeak && (
          <div className="flex flex-wrap gap-4 text-sm">
            <span className="text-zinc-400">
              {locale === 'ko' ? '최고 순위' : 'Peak rank'}{' '}
              <span className="text-pink-400 font-bold">#{bestPeak}</span>
            </span>
            <span className="text-zinc-400">
              {locale === 'ko' ? `${data.songs.length}곡` : `${data.songs.length} songs`}
            </span>
            <span className="text-zinc-400">
            ...
            <span className="text-zinc-400">
              {locale === 'ko' ? '총점 ' : 'Score '}
              <span className="text-pink-300 font-bold">{totalScore.toLocaleString()}</span>
            </span>
          </div>
        )}
      </div>
      ...
      {data.songs.length === 0 ? (
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
          <div className="grid grid-cols-[1fr_4rem_4rem_5rem_auto] gap-2 px-5 py-3 text-xs text-zinc-600 font-medium border-b border-zinc-800">
          ...
          <div className="divide-y divide-zinc-800/50">
            {data.songs.map((s, i) => (
              <div key={i} className="grid grid-cols-[1fr_4rem_4rem_5rem_auto] gap-2 items-center px-5 py-3">
